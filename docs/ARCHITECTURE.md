...
The local policy `osm-conservative-5` filters before pathfinding. Ordinary public road classes and cycleways can be candidates without explicit access tags; that is a documented inference. Explicit unknown or non-public access is rejected. Service roads and trails need explicit permissive access. No personal checkbox can turn a private road public.
...
## First-ride corrections

Policy `osm-conservative-5` changes Dijkstra state from `(node, incoming_way)` to `(node, incoming_edge_id, reverse)`. Incoming and outgoing directed-edge constraints span checkpoints, extension joins and lap closure. Immediate reversed edges and geometry joins of at least 160° are disallowed. Eight-metre geometry tangents are cached per edge; request-local turn results are cached to keep long searches bounded. Existing turn/access/major-road rules remain mandatory. Block and roundabout detours use mapped directed connectivity. `navigation.physical_uturns_per_lap` includes the lap join. A dead-end control can fail explicitly instead of producing a physical U-turn.

`access_blocks` stores private owner observations in SQLite: immutable ID, selected point, original highlighted section geometry, OSM references, note and timestamp. `/api/access-blocks/resolve` projects within 20 m; POST revalidates the selected section; DELETE reopens it. The highlighted geometry defines scope, not the whole way or an inferred estate. A metre-scaled spatial index removes overlapping edges in both directions before snapping/search. One metre of geometric tolerance handles projected split edges; a shared endpoint alone does not block a neighbour. Geometry-based matching continues to work if OSM way IDs change, but material map realignment still needs inspection. The tolerance is not a physical-location guarantee.

Personal blocks do not become scored public reports. General approved reports retain their scoring behaviour. `/api/routes/{id}/access` checks restored routes; every GPX export independently checks active blocks and policy. A records epoch invalidates in-flight searches/exports or access saves if observations or app records change. The UI retains an old route for reference when a replacement search fails and disables its exports when it crosses a block. Blocks are included in local data export and deletion.

`PhoneExport` prepares the selected OsmAnd GPX before the user clicks Share, preserving Web Share’s transient activation requirement. Files are keyed by route, lap selection, speed and access revision; changed access invalidates prepared files, including on return to the tab. Downloads use the same prepared file. Native file sharing is offered only when `navigator.canShare` accepts it; the user chooses the target. The optional QR flow opens a separate GPX-only HTTP listener on a selected local interface after an explicit “Create phone link” action. A 192-bit bearer token, 10-minute expiry, stop control, host checks and no-store headers limit the transfer. Changing the route/laps/speed/access revision closes the old link in the UI; server-side access changes and data deletion revoke it independently. The private API remains on loopback. Docker disables transfer because its dynamic ports are unpublished. See PHONE_TRANSFER_PROPOSAL.md for the exact scope and remaining physical-device check.
