...
Point routes use weighted Dijkstra and penalise used edges for alternatives. Manual loops sample distance/bearing bins, close within ±10% of the target and allow at most 18% repeated-edge distance. Best fit samples up to 72 representative pivots over several distance bands and directions, adds three-leg loops between branches, and evaluates closed loops of at least 2 km. It chooses `ceil(target / actual_lap_length)` laps, with totals 100–110% of the target and at most 100 laps. Nearby searches also retain valid local candidates so widening the sample cannot discard a better local option.

Candidates rank lexicographically by descending mapped-road score, ascending lap count, then distance above target. Up to three options show useful trade-offs: a lower-score option is retained only if it uses fewer laps than earlier options. Very similar alternatives are suppressed. This is a bounded candidate search, not proof of a globally safest or globally minimum-lap route. Empty results never relax restrictions.
